...
ShortestPathCalculator.prototype.makeSVG = function(elementId, width, height) {
	...
	this.graph.width  = width  ? width  : 500;
	this.graph.height = height ? height : 300;
...
ShortestPathCalculator.prototype.drawGraph = function(elementId, width, height) {
	...
	var force = d3.layout.force()
		...
		.links(this.paths)
		.charge(-500)
		.linkDistance(function(d){ return d.distance*0.2; }) // kali 2 untuk memperbesar jarak
		...
		.selectAll("circle")
			...
			.enter()
				.append("svg:circle")
					...
					.attr("r",  function(d) { return 5; }); // ukuran node
...
ShortestPathCalculator.prototype.formatResult = function() {
	...
	if(this.result.path === null)
		return "Tidak ada rute dari " + this.result.source + " ke " + this.result.target ;

	if(this.result.path.length === 0)
		return "Rute dari " + SpUtils.nodeNames[this.result.source] + " ke "
			+ SpUtils.nodeNames[this.result.target] + ". Tidak ada perjalanan. "
	...
	for(var i=0; i<this.result.path.length; i++) {
		...
		res += (i+1) + '. ' + sourceNode.value + ' -> ' + targetNode.value + '<br/>';
	}
	res += "<br/>";
	res += "Jarak tempuh : " + this.result.distance + " km  ";
...
ShortestPathCalculator.prototype.formatResultJarak = function() {

	
	var res = this.result.distance; 
	return res;
...
ShortestPathCalculator.prototype.formatResultDijkstra = function() {
	...
	for(var i=0; i<this.result.path.length; i++) {
		...
		var sourceNode = this.nodes[sourceNodeIndex];
		var targetNode = this.nodes[targetNodeIndex];
		res +=  sourceNodeIndex + '-' + targetNodeIndex + ', ';
	} 
...
ShortestPathCalculator.prototype.dijkstra = function(start, end) {
    ...
    if(shortestPath[end] < infinity) {
        ...
        return {mesg:'', path: newPath, source: start, target: end, distance:totalDistance};
    } 
    else {
        return {mesg:'Tidak ada rute yang disarankan.', path: null, source: start, target: end, distance: 0 };
    }
