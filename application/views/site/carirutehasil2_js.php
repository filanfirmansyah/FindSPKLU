...
function tambahCariTemp(hasildijkstra, totaljarak){ 
	...
	$.ajax({
		url: '<?php echo base_url(); ?>site/tambahcaritemp2/',
		type: 'post',
		data: fd,
		contentType: false,
		processData: false,
...
function initialize()
	...
	foreach ($data as $d) {
	 ...
	 // buat jalur hasil dijkstra
		...
		<?php  
		$rute2 = $this->session->userdata('hasildijkstra2'); 
		 
		$jalurs2=explode(",",$rute2);
		  
		 for($x=0; $x<sizeof($jalurs2); $x++) {
			$jalurs2[$x]=trim($jalurs2[$x]);
			
			$nodes = explode("-",$jalurs2[$x]);
			$node1=$nodes[0];
			$node2=$nodes[1];
