...
<?php
	$query=$this->db->query("SELECT * FROM titik 
		...
		foreach ($titik as $r) { 
			...
			$id = $r->id_titik;
		?>
		var distance = google.maps.geometry.spherical.computeDistanceBetween(new google.maps.LatLng(<?php echo $latitude; ?>, <?php echo $longitude; ?>), new google.maps.LatLng(document.getElementById('lat').value, document.getElementById('lon').value));
...
<?php 
		$query=$this->db->query("SELECT * FROM tempat WHERE id_tempat='".$this->uri->segment(3)."'");
		$tempat = $query->result();
...
<?php  
	...
	$jalurs2 = $query->result();
	$yyy = "";
	foreach ($jalurs2 as $jalur) { 
...
function tambahCariTemp(hasildijkstra, totaljarak){ 
	...
	$.ajax({
		url: '<?php echo base_url(); ?>site/tambahgetroute/',
		type: 'post',
		data: fd,
		contentType: false,
		processData: false,
...
function initialize1()
	...
	foreach ($data as $d) {
	 ...
	 // buat jalur hasil dijkstra
		...
		<?php  
		$rute = $this->session->userdata('hasilgetroute'); 
		...
		$jalurs=explode(",",$rute);
		 ...
		 for($x=0; $x<sizeof($jalurs); $x++) {
			...
			if($node1=="O") {
				...
				var lng1 = parseFloat(document.getElementById('lon').value);
...
function initialize()
	{
		...
		var type = 'markeranda'; 
		var loc = new google.maps.LatLng(document.getElementById('lat').value, document.getElementById('lon').value));
		var icon = customIcons[type] || {markeranda}; 
		var marker = new google.maps.Marker({
            map: map,
            position: loc,
